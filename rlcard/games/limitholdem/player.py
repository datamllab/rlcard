...
class LimitholdemPlayer(Player):
    ...
    def __init__(self, player_id):
        ...
        self.hand = []
        self.chips_remaining = 100
        self.status = 'alive'
        self.score = 0

        # below are for comparing cards 
        self._category = 0  # 0:"Not_Yet_Evaluated" 1: "High_Card" , 9:"Straight_Flush" etc
        self._five_cards = []  # the five best out of seven
        # self._hand_name = ""        # High_Card , One_Pair , Flush etc, same as catagory, no use now
        self._flush_cards = []  # self._flush_cards = ['4D','5D','6D','7D','8D','AD']
        # _cards_by_rank =  [[0], [1, '3C'], [1, '7C'], [1, '8D'], [1, '9S'], [2, 'TC', 'TD'], [1, 'JC']]
        self._cards_by_rank = []
        self._product = 1
    ...
    def _sort_cards(self):
        self._all_cards = sorted(
            self._all_cards, key=lambda card: self.RANK_LOOKUP.index(card[1]))

    def judge_score(self, cards):

        self._sort_cards()
        self._cards_by_rank, self._product = self._get_cards_by_rank(
            self._all_cards)
    ...
    def _get_flush_cards(self):
            ...
            for suit in self.SUIT_LOOKUP:
                ...
                if suit_count >= 5:
                    flush_cards = [
                        card for card in self._all_cards if card[1] == suit]
                    return flush_cards
            return []
