    def compare_hands(self):
        ...
        self.player0.hand.evaluate()
        self.player1.hand.evaluate()

        #compare hands
        player0_category = self.player0.hand.get_hand_category()
        player1_category = self.player1.hand.get_hand_category()
        if player0_category > player1_category:
            return 1
        elif player0_category < player1_category:
            return 2
        elif player0_category == player1_category:
            #compare equal category
            player0_5_cards = self.player0.get_hand_five_cards()
            player1_5_cards = self.player1.get_hand_five_cards()
            for i in reversed(range(5)):
                player0_card_rank = player0_5_cards[i][0]
                player1_card_rank = player1_5_cards[i][0]
                if RANKS.index(player0_card_rank) > RANKS.index(player1_card_rank):
                    return 1
                elif RANKS.index(player0_card_rank) < RANKS.index(player1_card_rank):
                    return 2
                else:
                    pass
...
# give player an attribute Hand
# an example below
class Player:
    def __init__(self,name = "Anonymous", chips = 1000 , face_up = False):
        self.name = name
        self.cards = [] #the hand now(self.hand = [] in Player.py)
        self.hand = Hand(name)
        self.chips = chips
    ...
    def add_card_to_hand(self,cards):
        for card in cards:
            self.hand.all_cards.append(card)
...
class Hand:
	def __init__(self, player_id):
		self.all_cards = []
		self.player_id = player_id            #   Player1_hand , Player2_hand
	...
	def _sort_cards(self):
		self._all_cards = sorted(self._all_cards ,key=lambda card: self.RANK_LOOKUP.index(card[1]))
	...
	def evaluateHand(self):
		...
		self._sort_cards()
		self.cards_by_rank , self.product = self._getcards_by_rank(self.all_cards)
	...
	def _getflush_cards(self) :
		...
		for suit in self.SUIT_LOOKUP:
			...
			if suit_count >= 5 :
				flush_cards = [card for card in self.all_cards if card[1]== suit]
				return flush_cards
		return []
