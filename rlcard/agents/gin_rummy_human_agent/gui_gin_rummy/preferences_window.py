...
class PreferencesWindow(object):
    ...
    def set_game_background_color(self):  # store because color cannot be obtained from its widget
        ...
        self.view.update_configuration_game_background_color(color=self.game_background_color)
    ...
    def set_new_values(self):
        ...
        window_size_factor = str(self.window_size_scale.get())
        configurations.WINDOW_SIZE_FACTOR = window_size_factor
        config.set(settings_section, window_size_factor_option, window_size_factor)

        with open('game_options.ini', 'w') as config_file:
            config.write(config_file)

    def on_cancel_button_clicked(self):
        self.view.update_configuration_game_background_color(color=self.original_game_background_color)
        self.pref_window.destroy()
